...
function resetToInitial(){
  ...
  ['dc_clients','dc_log','dc_history','dc_plans','dc_plantasks','dc_manual_done','dc_sms_days','dc_pay_disabled','dc_sheet_url'].forEach(k=>localStorage.removeItem(k+'__'+activeMonth));
...
function renderCalModal(cid){
  ...
  months.forEach(mk=>{
    ...
    for(let d=1;d<=daysInMonth;d++){
      ...
      const smsBtn=val?`<div onclick="event.stopPropagation();toggleDaySms('${cid}','${iso}')" style="position:absolute;bottom:2px;right:2px;width:14px;height:8px;border-radius:11px;background:${hasSms?'rgba(var(--accent-rgb),.5)':'rgba(255,255,255,.12)'};border:1px solid ${hasSms?'rgba(48,209,88,.7)':'rgba(255,255,255,.2)'};cursor:pointer;transition:all .15s;display:flex;align-items:center;" title="${hasSms?'SMS есть':'SMS нет'}"><div style="width:6px;height:6px;border-radius:10px;background:${hasSms?'var(--green)':'rgba(255,255,255,.4)'};margin-left:${hasSms?'7px':'1px'};transition:all .15s;flex-shrink:0"></div></div>`:'';
...
function undoLastCalendarChange(){ _sfx.play('undo');
  ...
  else{const{cid,name,iso,prev}=entry;if(!historyData[name])historyData[name]={};if(prev===''||prev===undefined)delete historyData[name][iso];else historyData[name][iso]=prev;saveAll();if(calCurrentCid===cid)renderCalModal(cid);showToast('↩ Отменено');}
...
function cycleCalDay(cid,iso){ _sfx.play('click');
  ...
  saveAll();renderCalModal(cid);updateSidebar();
}
