...
// Кнопки «убрать будущие отметки» больше нет: имейлы на будущие дни ты ставишь
// специально (в августе — на сентябрь), это и есть работа зоны. Импорт из таблицы,
// для которого кнопку и делали, будущие даты давно не заносит, а нажатие стёрло бы
// реальный заработок. Зону отметки теперь хранит метка (_markZone).

// Инструмент «убрать отметки из чужой зоны» удалён вместе с проблемой, которую он
// лечил. Отметка, поставленная в августе на сентябрьское число, — не ошибка: у каждой
// отметки есть метка зоны (_markZone), деньги за неё считает только эта зона, и
// удалять такие отметки нельзя — это реальный заработок августа.
